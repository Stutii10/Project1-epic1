*> =======================================================
       ...
       *> ------- File status variables -------
       ...
       01  ACCOUNTS-FULL-SHOWN           PIC X VALUE 'N'.
       ...
       READ-NEXT-INPUT.
           ...
           READ INPUT-FILE
               AT END
                   ...
                   PERFORM OUT-MSG
               NOT AT END
                   MOVE FUNCTION TRIM(InLine) TO InLine
                   IF InLine = SPACES
                       MOVE '0' TO WS-INPUT-VALUE
                   ELSE
           ...
           .
              ...
              *> Tell the user when the account store is full
       Notify-If-Full.
           IF Account-Count= 5 AND ACCOUNTS-FULL-SHOWN NOT = 'Y'
               MOVE "All permitted accounts have been created (5/5)." TO WS-MSG
               PERFORM OUT-MSG
               MOVE 'Y' TO ACCOUNTS-FULL-SHOWN
           END-IF
           .
      ...
      *>         PERFORM OUT-MSG
           ...
           *> --- Username step ---
           PERFORM UNTIL Username-Not-Exists
               MOVE "Create Account - Enter a username (max 20 chars)." TO WS-MSG
               PERFORM OUT-MSG
           ...
           *> --- Persist (already OPEN EXTEND) ---
           ...
           MOVE "Account created successfully. Please log in to continue." TO WS-MSG
           PERFORM OUT-MSG
           *> Do NOT set USER-LOGGED-IN here; return to auth loop
           PERFORM Notify-If-Full
           .
